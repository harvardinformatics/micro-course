...
```
33. Add gene symbols to results table
```{r,echo=TRUE}
...
sleuth_table_genes_level<-left_join(sleuth_table_genes_level,gene_and_symbol)
```
...
1. Open RSEM matrix
```{r,echo=TRUE}
rsem_gene_data<-read.table("dme_elevgrad_rsem_bt2_gene_counts.matrix",header=TRUE,row.names=1)
```
